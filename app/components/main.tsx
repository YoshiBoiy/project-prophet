...
export default function Main({ title, description, prediction }) {
	const src = "/images/" + title.replace(/\s+/g, '-').toLowerCase() + ".JPG"
	return (
		<div className="h-full">
			...
			<div className="overflow-x-auto flex flex-row h-1/4 -mt-20 py-20 gap-4 w-full">
				...
				<CarouselItem title="PE Ratio" number="21%" active={title == "PE Ratio"}></CarouselItem>
				<div className="h-36 w-4"></div>
			</div>
		</div>
	)
